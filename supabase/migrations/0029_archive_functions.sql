-- Migration 0029: Phase 3 — archive and unarchive functions.
...
BEGIN
  ...
  INSERT INTO contact_history (
    ...
    'manual',
    manager_uuid,
    'Lead unarchived by ' || COALESCE(manager_name, manager_uuid::text),
    false
  );
END;
$$ LANGUAGE plpgsql SECURITY DEFINER SET search_path = public;

CREATE OR REPLACE FUNCTION search_archived_leads_ids(search_term text)
...
BEGIN
  ...
  WHERE similarity(COALESCE(al.lead_name, ''), search_term) > 0.3
     OR similarity(al.lead_phone, search_term) > 0.3;
END;
$$ LANGUAGE plpgsql STABLE SECURITY DEFINER SET search_path = public;

COMMENT ON FUNCTION archive_single_lead IS 'Archives one lead: snapshot, migrate history, flag leads row.';
COMMENT ON FUNCTION unarchive_single_lead IS 'Restores archived lead to active CRM with history and audit row.';
COMMENT ON FUNCTION search_archived_leads_ids IS 'Trigram search over archived leads; manager-only.';

REVOKE ALL ON FUNCTION archive_single_lead(UUID, TEXT, TEXT, TEXT) FROM PUBLIC;
REVOKE ALL ON FUNCTION unarchive_single_lead(UUID, UUID) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION archive_single_lead(UUID, TEXT, TEXT, TEXT) TO service_role;
GRANT EXECUTE ON FUNCTION unarchive_single_lead(UUID, UUID) TO service_role;
GRANT EXECUTE ON FUNCTION search_archived_leads_ids(text) TO authenticated;
