-- Migration 0045: Lead preference fields and structured rec_hotel JSON output.

-- lead_details_safe depends on rec_hotel; drop before altering column type.
...
ALTER TABLE lead_details
  ADD CONSTRAINT lead_details_gender_check
...
ALTER TABLE lead_details
  ADD COLUMN room_category TEXT;

ALTER TABLE lead_details
  ADD CONSTRAINT lead_details_room_category_check
  CHECK (room_category = ANY (ARRAY['single'::text, 'double'::text, 'triple'::text, 'quad'::text]));

ALTER TABLE lead_details
  ADD COLUMN district_preference TEXT;

ALTER TABLE lead_details
  ALTER COLUMN rec_hotel TYPE jsonb USING NULL;
...
SELECT
  ...
  CASE
    WHEN is_manager_or_superadmin() THEN ld.gender
    WHEN EXISTS (
      SELECT 1 FROM leads l
      WHERE l.uuid = ld.lead_uuid
        AND l.assigned_to = auth.uid()
