/**
 * PMS change room/property — update purchased_room + optional place (Operator+).
 */
...
import { changeRoomType, PmsPlacementError } from '@/lib/pms/placement-ops';

const BodySchema = z.object({
  leadId: z.string().uuid(),
  newTypeId: z.string().uuid(),
...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  try {
    const result = await changeRoomType({
      leadId: parsed.data.leadId,
      newTypeId: parsed.data.newTypeId,
    ...
    const message = err instanceof Error ? err.message : 'Change room failed';
